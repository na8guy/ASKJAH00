...
def async_command_handler(func):
    """Decorator to make command handlers truly asynchronous"""
    @wraps(func)
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        return await loop.run_in_executor(
            None, 
            lambda: asyncio.run(func(update, context))
        )
    return wrapper
...
async def get_subscription_status_message(user: dict) -> str:
    ...
    if status == 'trial':
        ...
        return "âŒ No active subscription. Use /subscribe to start."
        ...
        if TWITTER_ENABLED:
            try:
                ...
                for tweet in twitter_data.get('data', []):
                    text = tweet['text']
                    # Simple sentiment analysis (replace with proper sentiment analysis)
                    positive_words = ['good', 'great', 'bull', 'moon', 'buy', 'up', 'rocket']
                    negative_words = ['bad', 'scam', 'rug', 'sell', 'down', 'dump']
                ...
                logger.warning(f"Twitter sentiment analysis failed: {str(e)}")
    ...
    try:
        ...
        if not token_data:
            await processing_msg.edit_text(
                "âŒ This token hasn't been tracked by the bot. "
        ...
        if not current_token:
            await processing_msg.edit_text("âŒ Failed to fetch current token data.")
            return ConversationHandler.END
        ...
        await update.message.reply_text(
            "âŒ Failed to complete analysis. The token data might be incomplete or corrupted."
        )
...
async def fetch_tokens_manual(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("âŒ An error occurred while fetching tokens. Please try again.")
...
async def trade_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        await update.message.reply_text("âŒ No user data found.")
        return
        
    status = (
        f"ðŸ“Š *Trade System Status*\n\n"
        f"ðŸ”„ Auto-fetch: {'âœ… Enabled' if user.get('last_token_check') else 'âŒ Disabled'}\n"
    ...
    try:
        ...
        if not user:
            ...
            await context.bot.send_message(
                chat_id=user_id,
                text="âŒ User account not found. Please use /start to create an account."
            )
            return
        ...
        if not user.get('solana') or not user['solana'].get('public_key'):
            ...
            await context.bot.send_message(
                chat_id=user_id,
                text="âŒ Wallet setup incomplete. Please use /set_wallet to complete wallet setup."
            )
            return
...
async def remove_illiquid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or 'portfolio' not in user:
        await update.message.reply_text("ðŸ“­ You don't have any positions.")
        return
    ...
    if removed_count > 0:
        await update.message.reply_text(
            f"ðŸ§¹ Removed {removed_count} illiquid positions from your portfolio."
        )
    else:
        await update.message.reply_text(
            "âœ… All your positions have sufficient liquidity."
        )
...
async def subscribe(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if sol_balance < SUBSCRIPTION_SOL_AMOUNT:
        await update.message.reply_text(
            f"âŒ Insufficient SOL balance. You need {SUBSCRIPTION_SOL_AMOUNT} SOL "
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("âŒ Cancel", callback_data='cancel_subscription')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        try:
            ...
            if tx_status.value and not tx_status.value.transaction.meta.err:
                ...
                try:
                    await context.bot.send_message(
                        chat_id=user_id,
                        text="âŒ Your subscription payment failed. Please try /subscribe again."
                    )
                except:
                    pass
        except Exception as e:
...
async def generate_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user and user.get('solana', {}).get('public_key'):
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("âŒ Cancel", callback_data='cancel_new_wallet')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def confirm_generate_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE, new_user=False) -> int:
    ...
    try:
        ...
        error_msg = f"âŒ Error generating wallet: {str(e)}"
...
async def set_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("âŒ Cancel", callback_data='cancel_import')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    try:
        ...
        if user and user.get('solana') and user['solana'].get('public_key'):
            ...
            keyboard = [[InlineKeyboardButton("âœ… Confirm", callback_data='confirm_set_wallet')],
                        [InlineKeyboardButton("âŒ Cancel", callback_data='cancel_set_wallet')]]
            reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        error_msg = f"âŒ Failed to import wallet: {str(e)}"
        if "Invalid mnemonic phrase" in str(e):
            error_msg = "âŒ Invalid mnemonic phrase. Please check your words and try again."
...
async def input_private_key(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        error_msg = await update.message.reply_text(
            f"âŒ Invalid private key: {str(e)}\n\n"
            "Valid formats:\n"
...
async def confirm_set_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    private_key=wallet_input if method == 'private_key' else None
        ...
        await query.edit_message_text(
            f"âŒ Failed to import wallet: {str(e)}\n\n"
...
async def set_auto_buy_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if amount <= 0:
            await update.message.reply_text("âŒ Please enter a positive amount.")
            return SET_AUTO_BUY_AMOUNT
        ...
        await update.message.reply_text("âŒ Invalid amount. Please enter a number.")
        return SET_AUTO_BUY_AMOUNT
...
async def set_sell_percentage(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        [InlineKeyboardButton("âœ… Use Recommended Multi-Tier", callback_data='default_strategy')],
        [InlineKeyboardButton("âš™ï¸ Customize Strategy", callback_data='custom_strategy')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await update.message.reply_text(
        "ðŸ“Š *Profit-Taking Strategy*\n\n"
...
async def set_profit_strategy(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == 'default_strategy':
        ...
        await query.edit_message_text(
            "âœ… Using recommended multi-tier profit strategy.\n\n"
        ...
        await query.edit_message_text(
            "âš™ï¸ *Custom Profit Strategy Setup*\n\n"
...
async def set_custom_profit_targets(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if high_risk_targets:
            risk_warning = "âš ï¸ Warning: Profit targets over 200% are very ambitious and rarely hit.\n"
        ...
        await update.message.reply_text(
            f"{risk_warning}âœ… Custom profit strategy set.\n\n"
            "ðŸ“‰ Enter the stop-loss percentage (e.g., 10 for 10% loss):",
            parse_mode='Markdown'
        )
        return SET_LOSS_PERCENTAGE
...
async def set_loss_percentage(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if percentage <= 0:
            await update.message.reply_text("âŒ Stop-loss must be a positive percentage.")
            return SET_LOSS_PERCENTAGE
            
        if percentage > 50:
            await update.message.reply_text(
                "âš ï¸ Warning: A stop-loss over 50% is very risky. "
        ...
        await update.message.reply_text("âŒ Invalid percentage. Please enter a number.")
        return SET_LOSS_PERCENTAGE
...
async def confirm_high_risk_loss(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if response not in ['yes', 'y']:
        await update.message.reply_text(
            "ðŸ›‘ High-risk stop-loss cancelled.\n\n"
            "ðŸ“‰ Enter a new stop-loss percentage (e.g., 10 for 10% loss):"
        )
        return SET_LOSS_PERCENTAGE
    ...
    await update.message.reply_text(
        "âš ï¸ High-risk stop-loss enabled. Trade carefully!\n\n"
        "ðŸ”’ Enable anti-MEV protection? (yes/no)\n\n"
        "Anti-MEV measures help reduce the risk of front-running and sandwich attacks."
    )
    return SET_ANTI_MEV
...
async def set_anti_mev(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if response not in ['yes', 'no', 'y', 'n']:
        await update.message.reply_text("âŒ Please answer with 'yes' or 'no'.")
        return SET_ANTI_MEV
...
async def review_config_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == 'activate_auto_trading':
        ...
        await query.edit_message_text(
            "âœ… Auto-trading activated! The bot will now automatically trade tokens based on your settings."
        )
        return ConversationHandler.END
        ...
        await query.edit_message_text(
            "âš™ï¸ Let's configure your auto-trading settings again.\n\n"
            "Choose trading mode (Solana only):",
...
async def set_volume_threshold(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        threshold = float(update.message.text)
        if threshold < 500:
            await update.message.reply_text("âŒ Minimum volume should be at least $500 for safety.")
            return SET_VOLUME_THRESHOLD
...
async def set_max_slippage(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if slippage < 1 or slippage > 20:
            await update.message.reply_text("âŒ Slippage should be between 1% and 20%.")
            return SET_MAX_SLIPPAGE
...
async def set_max_gas_price(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if gas_price <= 0 or gas_price > 0.01:
            await update.message.reply_text("âŒ Gas price should be between 0.0001 and 0.01 SOL.")
            return SET_MAX_GAS_PRICE
...
async def set_token_age(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if min_age < 1 or min_age > 120:
            await update.message.reply_text("âŒ Token age should be between 1 and 120 minutes.")
            return SET_TOKEN_AGE
        ...
        await update.message.reply_text(
            "ðŸ”¢ Set the maximum number of open positions (e.g., 5):\n\n"
            "This limits how many different tokens you'll hold at once."
        )
        return SET_MAX_POSITIONS
        
    except ValueError:
        await update.message.reply_text("âŒ Invalid number. Please enter a number.")
        return SET_TOKEN_AGE
...
async def set_max_positions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if max_positions < 1 or max_positions > 20:
            await update.message.reply_text("âŒ Maximum positions should be between 1 and 20.")
            return SET_MAX_POSITIONS
        ...
        keyboard = [
            [InlineKeyboardButton("âœ… Activate Auto-Trading", callback_data='activate_auto_trading')],
        ...
        await update.message.reply_text("âŒ Invalid number. Please enter a whole number.")
        return SET_MAX_POSITIONS
...
def generate_config_summary(user):
    ...
    summary = (
        f"ðŸ¤– *Auto-Trading Configuration Review*\n\n"
        f"ðŸ”§ *Settings Summary:*\n"
        f"â€¢ Auto-buy amount: {user['auto_buy_amount']} SOL\n"
        f"â€¢ Min liquidity: ${user.get('min_liquidity', 1000):,.2f}\n"
        f"â€¢ Min volume: ${user.get('min_volume', 500):,.2f}\n"
        f"â€¢ Min token age: {user.get('min_token_age', 10)} minutes\n"
        f"â€¢ Max slippage: {user.get('max_slippage', 5)}%\n"
        f"â€¢ Max positions: {user.get('max_positions', MAX_POSITIONS)}\n"
    ...
    if user.get('profit_strategy') == 'multi_tier':
        summary += (
            f"ðŸ“Š *Profit Strategy:* Multi-tier (Recommended)\n"
            f"   - 40% at +20% profit\n"
            f"   - 30% at +50% profit\n"
            f"   - 30% at +100% profit\n"
        ...
        summary += f"ðŸ“Š *Profit Strategy:* Custom\n"
    ...
    # Add risk assessment
    summary += f"âš ï¸ *Risk Assessment:* {risk_score}/100\n"
    
    if risk_score > 70:
        summary += "ðŸš¨ High risk configuration detected! Consider adjusting your settings.\n\n"
    elif risk_score > 40:
        summary += "âš ï¸ Moderate risk configuration. Trade carefully.\n\n"
    else:
        summary += "âœ… Conservative configuration. Lower risk profile.\n\n"
...
async def input_contract(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not contract_address:
        await update.message.reply_text("âŒ Contract address cannot be empty. Please enter a valid Solana token address.")
    ...
    try:
        ...
        await update.message.reply_text(
            "âŒ Invalid contract address format. Please enter a valid Solana token address (e.g., 4k3Dyjzvzp8eMZWUXbBCjEvwSkkk59S5iCNLY3QrkX6R)."
        )
    ...
    if not token:
        await update.message.reply_text(
            "âŒ Failed to fetch token data. The token may not have active trading pairs or the address is invalid."
        )
...
async def job_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.job_queue:
        await update.message.reply_text("âŒ Job queue not initialized")
        return
...
async def handle_token_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    token = await fetch_token_by_contract(contract_address)
    if not token:
        await query.message.reply_text("âŒ Failed to fetch token details. Please try again.")
        log_user_action(user_id, "TOKEN_FETCH_FAILED", contract_address, level="warning")
        return
    ...
    if action == 'buy':
        ...
        if contract_address not in portfolio:
            await query.message.reply_text(f"âŒ You don't hold any {token['name']} tokens to sell.")
            return ConversationHandler.END
...
async def buy_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if not token:
            await update.message.reply_text("âŒ Token information missing. Please start the trade again.")
            return ConversationHandler.END
        ...
        if balance < amount:
            await update.message.reply_text(
                f"âŒ Insufficient SOL balance. Your balance: {balance:.4f} SOL\n"
        ...
        if success:
            ...
            await update.message.reply_text("âŒ Trade failed. Please try again later.")
...
async def sell_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if token['contract_address'] not in portfolio:
            await update.message.reply_text(f"âŒ You don't hold any {token['name']} tokens to sell.")
            return ConversationHandler.END
        ...
        if token_amount > token_amount_held:
            await update.message.reply_text(
                f"âŒ Insufficient token balance. Available: {token_amount_held:.2f} tokens\n"
        ...
        if success:
            ...
            await update.message.reply_text(f"âŒ {message}")
...
async def confirm_trade(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if success:
        ...
        await query.message.reply_text("âŒ Trade failed. Please try again.")
...
async def auto_cleanup_zero_balance(context: ContextTypes.DEFAULT_TYPE):
    ...
    if removed_count > 0:
        ...
        try:
            await context.bot.send_message(
                chat_id=user_id,
                text=f"ðŸ§¹ Auto-removed {removed_count} zero-balance positions from your portfolio."
            )
        except Exception as e:
...
async def mypositions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for contract, pos in portfolio.items():
        ...
        if pos['amount'] <= 0.001:  # Consider very small amounts as zero
            ...
            message = (
                f"ðŸ§¹ *Zero Balance: {pos['name']} ({pos['symbol']})*\n\n"
                f"â€¢ Initial Investment: {pos['amount']:.4f} SOL\n"
                f"â€¢ Current Value: 0.0000 SOL\n"
                f"â€¢ Status: âŒ No longer holding any tokens\n\n"
                f"ðŸ”— Contract: `{contract}`"
            )
            
            # Create action buttons - only remove for zero balance
            keyboard = [
                [InlineKeyboardButton("ðŸ—‘ï¸ Remove", callback_data=f"remove_{contract}")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        status = "âœ…" if is_active else "âŒ (Illiquid)"
        ...
        message = (
            f"ðŸ“Š *{pos['name']} ({pos['symbol']})* {status}\n\n"
            f"â€¢ Current Price: ${current_price:.8f}\n"
    ...
    summary = (
        ...
        f"â€¢ Active Positions: {len(portfolio) - zero_balance_positions}\n"
        f"â€¢ Zero Balance Positions: {zero_balance_positions}\n"
...
async def cleanup_zero_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if removed_count > 0:
        await update.message.reply_text(
            f"ðŸ§¹ Removed {removed_count} zero-balance positions: {', '.join(removed_names)}"
        )
        log_user_action(user_id, "ZERO_BALANCE_CLEANUP", f"Removed {removed_count} positions")
    else:
        await update.message.reply_text(
            "âœ… No zero-balance positions found."
        )
...
async def handle_remove_position(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if token_balance <= 0.001:
        message = (
            f"ðŸ—‘ï¸ Remove zero-balance position: {token_name}\n\n"
            f"This position has a balance of {token_balance:.4f} SOL.\n"
            f"Removing it will clean up your portfolio."
        )
    else:
        message = (
            f"ðŸ—‘ï¸ Remove position: {token_name}\n\n"
            f"Current balance: {token_balance:.4f} SOL\n"
            f"Are you sure you want to remove this position?\n\n"
            f"âš ï¸ This will delete all records of this position from your portfolio."
        )
    
    # Create confirmation keyboard
    keyboard = [
        [
            InlineKeyboardButton("âœ… Confirm Remove", callback_data=f"confirm_remove_{contract_address}"),
            InlineKeyboardButton("âŒ Cancel", callback_data=f"cancel_remove_{contract_address}")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_remove_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'confirm':
        ...
        await query.edit_message_text(
            f"ðŸ—‘ï¸ Successfully removed {token_name} from your portfolio.",
            parse_mode='Markdown'
        )
        ...
        # Cancel removal
        await query.edit_message_text(
            "ðŸ›‘ Removal cancelled. The token remains in your portfolio.",
            parse_mode='Markdown'
        )
...
async def handle_position_actions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    token = await fetch_token_by_contract(contract_address)
    if not token:
        await query.message.reply_text("âŒ Could not fetch token data.")
        return
    ...
    if action == 'sellpos':
        ...
        if contract_address not in user.get('portfolio', {}):
            await query.message.reply_text("âŒ You don't hold this token.")
            return
...
async def cleanup_illiquid_positions(context: ContextTypes.DEFAULT_TYPE):
    ...
    for contract in illiquid_tokens:
        ...
        try:
            await context.bot.send_message(
                chat_id=user_id,
                text=f"ðŸ§¹ Removed illiquid position: {position.get('name', 'Unknown')} (${position.get('liquidity', 0):.2f} liquidity)"
            )
        except Exception as e:
...
async def settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        try:
            ...
            await update.message.reply_text(f"âœ… Maximum positions updated to {new_max_positions}.")
        except ValueError:
            await update.message.reply_text("âŒ Please provide a valid number.")
    else:
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update and update.message:
        try:
            await update.message.reply_text(
                "âŒ An error occurred. Please try again or contact support.\n"
