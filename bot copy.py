...
try:
    ...
    raise ImportError(f"Missing required package: {str(e)}. Please install dependencies with: pip install 'python-telegram-bot[job-queue]' requests solana solders python-dotenv web3 base58 mnemonic pymongo cryptography httpx")
...
try:
    ...
    raise ConnectionFailure(f"Failed to connect to MongoDB Atlas: {str(e)}")

# Master key for encryption key derivation
    ...
    if not user:
        ...
        try:
            ...
            message = await update.message.reply_text(
                ...
                f"This wallet is tied to your Telegram ID. To generate a new wallet, use /generate_wallet.\n"
                f"To import an existing wallet, use /set_wallet.\n"
                f"Use /setmode to choose trading mode (Solana only), /balance to check your wallet, or /transfer to send tokens."
            )
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                f"You already have a wallet. To generate a new one, use /generate_wallet.\n"
                f"To import an existing wallet, use /set_wallet.\n"
                f"Use /setmode to choose trading mode, /balance to check your wallet, or /transfer to send tokens."
            )
        except Exception as e:
            await update.message.reply_text(
                f"Error accessing wallet data: {str(e)}. Please try /generate_wallet to create a new wallet or /set_wallet to import one."
...
async def set_loss_percentage(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if context.job_queue is None:
            await update.message.reply_text(
                "Error: JobQueue is not available for automatic trading. Install 'python-telegram-bot[job-queue]'."
            )
...
async def execute_trade(user_id, contract_address, amount, action, chain):
    ...
    try:
        ...
        if action == 'sell':
            input_token, output_token = output_token, input_token  # Swap tokens for sell
...
async def execute_transfer(user_id, recipient, token_contract, amount, chain):
    ...
    return True  # Keep simulation for transfers as GMGN API is for swaps

async def auto_trade(context: ContextTypes.DEFAULT_TYPE, user_id, token):
    user = users_collection.find_one({'user_id': user_id})
...
def main():
    try:
        ...
        # Define bot commands for the Telegram command menu
        commands = [
            BotCommand("start", "Start the bot and create a new wallet"),
            BotCommand("generate_wallet", "Generate a new wallet (overwrites existing)"),
            BotCommand("set_wallet", "Import an existing wallet using mnemonic or private key"),
            BotCommand("setmode", "Set trading mode (manual or automatic) for Solana"),
            BotCommand("trade", "Manually trade Solana tokens"),
            BotCommand("balance", "Check your wallet balances and token holdings"),
            BotCommand("transfer", "Transfer Solana tokens to another address"),
            BotCommand("reset_tokens", "Reset the list of posted tokens to receive all tokens again")
        ]
        ...
        logger.debug("Adding handlers to application")
